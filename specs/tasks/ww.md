...
- for each group, get the text value of each <p>, then:
  ...
  - if the group has only 1 item, set it as the current Category
  - if the group is in the format Analyte, Result, RL, Units, Analyzed, Qualifier (optional), add it (as an object, with category = current Category) to the Analytes list
